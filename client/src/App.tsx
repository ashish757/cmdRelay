...
function MainInterface() {
  const { activeLayout, setIsMenuOpen, isMenuOpen, activePage } = useUI();

  return (
      <main className="h-screen w-screen bg-black flex flex-col overflow-hidden select-none touch-none text-white font-sans ">
        <ConnectionScreen />
        <MenuOverlay />

          {
              isMenuOpen ? (
                      <button
                          ...
                          className="absolute top-4 right-6 z-50 w-6 h-6 text-3xl"
                      >
                          X
                      </button>
            ...
            <ControlLayoutRenderer activeLayout={activeLayout}/>
             {activePage == "builder" && <ControlLayoutBuilder />}
      </main>
  );
}
